...
#
# Usage:
#   bash run.sh start        # launch the dynamic-RRF run in the background
...
# results/answers_hybrid_dynamic_rrf.jsonl grows one row per completed query (flushed
...
LOG_DIR="logs"
TOTAL_ROWS=50
RESULTS="results/answers_hybrid_dynamic_rrf.jsonl"
...
case "${1:-}" in
  start)
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/dynamic_rrf_$(date +%Y-%m-%d_%H-%M-%S).log"
