...
#
# Usage:
#   bash run.sh start        # launch the query-type-conditioned run in the background
...
# results/answers_hybrid_query_type.jsonl grows one row per completed query (flushed
...
LOG_DIR="logs"
TOTAL_ROWS=50
RESULTS="results/answers_hybrid_query_type.jsonl"
...
case "${1:-}" in
  start)
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/query_type_$(date +%Y-%m-%d_%H-%M-%S).log"
